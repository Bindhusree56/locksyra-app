    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur rounded-3xl p-6 shadow-lg border-2 border-cyan-200">
        ...
        <div className="flex gap-3 mb-4 flex-wrap">
          <span className="px-3 py-1 bg-cyan-50 border border-cyan-200 text-cyan-700 text-xs rounded-full font-medium">{passwords.length} saved</span>
          <span className="px-3 py-1 bg-green-50 border border-green-200 text-green-700 text-xs rounded-full font-medium">{passwords.filter((p) => p.strength?.level === 'strong' || p.strength?.level === 'excellent').length} strong</span>
          <span className="px-3 py-1 bg-red-50 border border-red-200 text-red-700 text-xs rounded-full font-medium">{passwords.filter((p) => p.strength?.level === 'weak').length} weak</span>
        </div>
        ...
        {showForm && (
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-2xl p-5 border-2 border-cyan-200 mb-5">
            ...
            <div className="space-y-3">
              ...
              {newEntry.password && (() => {
                const str = analyzePasswordStrength(newEntry.password);
                const col = strengthColors[str.level];
                return (
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                      <div className={`h-full ${col.bar} transition-all`} style={{ width: `${str.score}%` }} />
                    </div>
        ...
        {auditResults && (
          <div className="bg-purple-50 rounded-2xl p-5 border-2 border-purple-200 mb-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setAuditResults(null)}><X className="w-4 h-4 text-purple-400" /></button>
            </div>
        ...
        <div className="space-y-3">
          {passwords.length === 0 ? (
            ...
            passwords.map((entry) => {
              ...
              return (
                <div key={entry.id}
                  className={`rounded-2xl p-4 border-2 transition-all ${isEditing ? 'bg-blue-50 border-blue-300' : 'bg-gray-50 border-gray-200 hover:border-cyan-300'}`}>
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      {/* Username */}
                      {isEditing ? (
                        <input value={editData.username} onChange={(e) => updateEditField('username', e.target.value)}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => copyToClipboard(isEditing ? editData.password : entry.password)}
                          className="p-1.5 bg-cyan-100 text-cyan-700 rounded-lg hover:bg-cyan-200">
                      ...
                      {isEditing && editData.password && (
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                            <div className={`h-full ${strengthColors[str.level]?.bar} transition-all`} style={{ width: `${str.score}%` }} />
                          </div>
                          <span className="text-xs text-gray-500 capitalize">{str.level}</span>
                        </div>
                      )}
                      ...
                      {isEditing ? (
                        <textarea value={editData.notes} onChange={(e) => updateEditField('notes', e.target.value)}
                          placeholder="Notes (optional)"
                          className="w-full px-3 py-1.5 rounded-lg border-2 border-blue-300 outline-none text-xs bg-white" rows={2} />
                      ) : (
