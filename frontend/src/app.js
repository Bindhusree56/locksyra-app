...
import { Shield, Newspaper, Key, BookOpen, Settings, Home, Mail } from 'lucide-react';
              ...
              <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                {[
                  ...
                  <button key={item.id} onClick={() => handleNavigation(item.id)}
                    className={`bg-gradient-to-r ${item.color} border-2 rounded-2xl p-4 hover:shadow-lg transition-all text-left`}>
                    {item.icon}
